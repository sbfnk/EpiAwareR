...
# Skip EpiNow2 fitting on CI (too slow and times out)
# Users can run locally for full comparison
run_epinow2 <- !nzchar(Sys.getenv("CI"))
```
...
**Note:** EpiNow2 fitting is skipped on CI due to runtime constraints. Run locally for the full comparison.

```{r epinow2_example, message = FALSE, eval = run_epinow2}
library(EpiNow2)
...
```{r runtime_comparison, eval = run_epinow2}
