...
`epiaware_fit` objects integrate directly with the
[posterior](https://mc-stan.org/posterior/) package. You can convert the
samples to any draws format for use with bayesplot, loo, or other packages:
...
subset_draws(draws, variable = c("ar_damp.1.", "ar_damp.2.")) |>
  summarise_draws()
```

This also enables [bayesplot](https://mc-stan.org/bayesplot/) diagnostics:

```{r bayesplot_trace, message = FALSE}
library(bayesplot)

# Trace plots for AR damping coefficients
mcmc_trace(as_draws_array(results),
           pars = c("ar_damp.1.", "ar_damp.2."))
```
...
```{r compare_ar_plot, fig.width=10, fig.height=4}
library(ggplot2)
library(patchwork)
...
```{r compare_delays_plot, fig.width=10, fig.height=4}
